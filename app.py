...
@app.route('/api/pinterest/search', methods=['GET'])
def pinterest_search():
    ...
    if 'items' not in data:
        return jsonify({'status': 'error', 'error': str(data)}), 400

    images = []
    for pin in data['items']:
        img = pin.get('media', {}).get('images', {})
        url = (img.get('600x', {}) or img.get('400x300', {}) or {}).get('url')
        if url:
            images.append({'id': pin['id'], 'url': url, 'title': pin.get('title', '')})
